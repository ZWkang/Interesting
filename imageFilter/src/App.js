...
import imageScreenShot, { download } from "image-screenshot";
import {
  LayoutContainer,
  LayoutLeft,
  LayoutRight,
...
class App extends Component {
    ...
    return () => {
      ...
      html2canvas(saveRef)
        .then(canvas => {
          ...
          download(canvas, "ScreenShot." + type);
  ...
  BtnGroupdAppendChild = ({ resetState, resetImage }) => {
    return (
      <React.Fragment>
  ...
  render() {
    ...
    return (
      ...
      >
        <Page center maxWidth="70vw">
          <Section>
            <MyDropzone ref="dropzone" filterValue={this.state.filterValue} />
          </Section>
          <Section>
            <BtnGroups
              saveAs={this.saveAs}
